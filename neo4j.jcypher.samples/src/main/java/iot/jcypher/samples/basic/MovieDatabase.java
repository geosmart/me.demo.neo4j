...
/**
 * This JCypher sample is constructing and querying the 'Movie Database'. It is based on the 'Movie
 * Database' sample, provided in the Neo4j Manual in the Tutorials section
 */
public class MovieDatabase {
  ...
  static void createMovieDatabaseByQuery() {
	...
	query.setClauses(new IClause[] {CREATE.node(matrix1).label("Movie").property("title").value("The Matrix").property("year").value("1999-03-31"),
		CREATE.node(matrix2).label("Movie").property("title").value("The Matrix Reloaded").property("year").value("2003-05-07"),
		CREATE.node(matrix3).label("Movie").property("title").value("The Matrix Revolutions").property("year").value("2003-10-27"),
		CREATE.node(keanu).label("Actor").property("name").value("Keanu Reeves").property("like").value(8.5).property("numbers").value(1, 2, 3),
		CREATE.node(laurence).label("Actor").property("name").value("Laurence Fishburne").property("like").value(7),
		CREATE.node(carrieanne).label("Actor").property("name").value("Carrie-Anne Moss").property("like").value(8.3),
	...
	/**
	 * map to CYPHER statements and map to JSON, print the mapping results to System.out. This will
	 * show what normally is created in the background when accessing a Neo4j database
	 */
  ...
  static void createAdditionalNodes() {
	...
	query.setClauses(new IClause[] {
		CREATE.node(escape_plan).label("Movie").property("title").value("Escape Plan").property("year").value("2013-03-31"),
		CREATE.node(arnie).label("Actor").property("name").value("Arnold Schwarzenegger").property("like").value(9.0),
		CREATE.node(sylvester).label("Actor").property("name").value("Sylvester Stalone").property("like").value(9.0),
  ...
  static void queryNodeCount() {
	...
	query = new JcQuery();
	query.setClauses(new IClause[] {MATCH.node(n), RETURN.count().value(n).AS(nCount)});
	/**
	 * map to CYPHER statements and map to JSON, print the mapping results to System.out. This will
	 * show what normally is created in the background when accessing a Neo4j database
	 */
  ...
  static void queryMovieGraph() {
	...
	/*******************************/
	JcQuery query = new JcQuery();
	query.setClauses(new IClause[] {MATCH.node(actor).label("Actor").relation().out().type("ACTS_IN").node(movie), RETURN.value(actor),
		RETURN.value(movie)});
	/**
	 * map to CYPHER statements and map to JSON, print the mapping results to System.out. This will
	 * show what normally is created in the background when accessing a Neo4j database
  ...
  private static void initDBConnection() {
	...
	props.setProperty(DBProperties.DATABASE_DIR, System.getProperty("user.dir") + "//db//NEO4J_DBS//01");
